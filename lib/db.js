...
const DATABASE_FILE = process.env.DATABASE_FILE || path.join(__dirname, '../data.db');
...
function addHistoryEntry(data) {
    const stmt = db.prepare(`
        INSERT INTO history (date, action, staked, unstaked, totalTrx, trxPrice, txids)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    stmt.run(
        ...
        data.totalTrx,
        data.trxPrice || null,
        data.txids ? JSON.stringify(data.txids) : null
    );
}
