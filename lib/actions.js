const { tronWeb, secondaryTronWeb, CONFIG, log, sleep, executeTronCall } = require('./config');
...
async function claimRewards() {
    ...
    try {
        ...
        const transaction = await executeTronCall(client => client.transactionBuilder.withdrawBlockRewards(address));
        const signedTx = await executeTronCall(client => client.trx.sign(transaction));
        const receipt = await executeTronCall(client => client.trx.sendRawTransaction(signedTx));
...
async function stakeEnergy() {
    ...
    try {
        const address = tronWeb.defaultAddress.base58;
        const balanceSun = await executeTronCall(client => client.trx.getBalance(address));
        ...
        if (amountToStakeSun >= 1_000_000) {
            ...
            const stakeTx = await executeTronCall(client => client.transactionBuilder.freezeBalanceV2(amountToStakeSun, 'ENERGY', address));
            const signedStakeTx = await executeTronCall(client => client.trx.sign(stakeTx));
            const receipt = await executeTronCall(client => client.trx.sendRawTransaction(signedStakeTx));
...
async function voteSR() {
    ...
    try {
        const address = tronWeb.defaultAddress.base58;
        const resources = await executeTronCall(client => client.trx.getAccountResources(address));
        ...
        if (voteAmount > 0) {
            ...
            const voteTx = await executeTronCall(client => 
                (client.transactionBuilder.vote ? 
                    client.transactionBuilder.vote({ [CONFIG.VOTE_SR_ADDRESS]: voteAmount }, address) : 
                    client.transactionBuilder.voteWitness({ [CONFIG.VOTE_SR_ADDRESS]: voteAmount }, address))
            );
            const signedVoteTx = await executeTronCall(client => client.trx.sign(voteTx));
            const receipt = await executeTronCall(client => client.trx.sendRawTransaction(signedVoteTx));
...
async function delegateEnergy(targetWallet) {
    ...
    try {
        ...
        const account = await executeTronCall(client => client.trx.getAccount(address));
        await sleep(10000);
        
        const resources = await executeTronCall(client => client.trx.getAccountResources(address));
        await sleep(10000);
        
        const canDelegateRes = await executeTronCall(client => client.trx.getCanDelegatedMaxSize(address, 'ENERGY')).catch(() => ({ max_size: 0 }));
        ...
        if (availableToDelegateSun >= 1_000_000) {
            ...
            const delegateTx = await executeTronCall(client => client.transactionBuilder.delegateResource(availableToDelegateSun, targetWallet, 'ENERGY', address));
            const signedDelegateTx = await executeTronCall(client => client.trx.sign(delegateTx));

            log('Transaction signed. Sending...');
            
            const receipt = await executeTronCall(client => client.trx.sendRawTransaction(signedDelegateTx));
