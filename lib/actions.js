    ...
    try {
        ...
        if (account.frozenV2) {
            account.frozenV2.forEach(item => {
                if (item.type === 'ENERGY') {
                    energyStakedSun += (item.amount || 0);
                }
            });
        }

        let delegatedSun = account.delegated_frozenV2_balance_for_energy || account.delegated_frozen_balance_for_energy || 0;
        const availableToDelegateSun = Math.max(0, energyStakedSun - delegatedSun);
        log(`Calculation done. Staked: ${energyStakedSun}, Delegated: ${delegatedSun}, Available: ${availableToDelegateSun}`);
        ...
        if (availableToDelegateSun >= 1_000_000) {
            ...
            log(`Receipt received: ${JSON.stringify(receipt)}`);
