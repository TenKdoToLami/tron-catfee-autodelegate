const { tronWeb, secondaryTronWeb, CONFIG, log, sleep } = require('./config');
...
async function delegateEnergy(targetWallet) {
    ...
    try {
        ...
        try {
            ...
            canDelegateRes = await tronWeb.trx.getCanDelegatedMaxSize(address, 'ENERGY');
        } catch (err) {
            ...
            await sleep(5000);
            canDelegateRes = await secondaryTronWeb.trx.getCanDelegatedMaxSize(address, 'ENERGY').catch(() => ({ max_size: 0 }));
        }
