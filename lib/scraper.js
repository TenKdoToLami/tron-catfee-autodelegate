...
async function fetchCatfeeWithPuppeteer(url = 'https://catfee.io/api/stake/public/project', timeoutMs = 40000) {
    ...
    try {
        console.log('[Puppeteer] Launching headless browser with stealth...');
        ...
        while (Date.now() - startTime < timeoutMs) {
            ...
            if (!pageTitle.includes('Just a moment')) {
                ...
                // If still stuck on challenge after 5s, attempt to interact with the Turnstile checkbox
                if (!attemptedClick && (Date.now() - startTime > 5000)) {
                    attemptedClick = await trySolveTurnstileCheckbox(page);
                    if (attemptedClick) {
                        console.log('[Puppeteer] Interacted with Turnstile checkbox.');
                    }
                }
            }
        ...
        if (!rawJson) {
            ...
            console.warn(`[Puppeteer] Timed out waiting for Cloudflare challenge. Final page title: "${finalTitle}"`);
            return null;
        }
